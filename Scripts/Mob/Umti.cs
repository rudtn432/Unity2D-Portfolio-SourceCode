...
public class Umti : MobData
{
    Animator animator;
    private protected Rigidbody2D rigidbody;
    private protected Vector2 movement;

    [SerializeField] private GameObject prfEffAtt;     // att 이펙트
    [SerializeField] private Spit prfSpit;
    ...
    private void FixedUpdate()
    {
        if (hp > 0)
        {
            if (!animator.GetCurrentAnimatorStateInfo(0).IsName("att") && !animator.GetCurrentAnimatorStateInfo(0).IsName("die"))
                PlayerTracking();
        }
    }
    ...
    IEnumerator Att()      // 독침 뱉기
    {
        ...
        {
            GameObject instance = Instantiate(prfEffAtt, new Vector2(transform.position.x + 0.367f, transform.position.y - 0.1f), Quaternion.identity);
            instance.transform.localScale = new Vector3(-1, 1, 1);
            Instantiate(prfSpit, new Vector2(transform.position.x + 0.367f, transform.position.y - 0.1f), Quaternion.identity);

        }
        else
        {
            Instantiate(prfEffAtt, new Vector2(transform.position.x - 0.367f, transform.position.y - 0.1f), Quaternion.identity);
            Instantiate(prfSpit, new Vector2(transform.position.x - 0.367f, transform.position.y - 0.1f), Quaternion.identity);
        }
    ...
    private void OnEnable()
    {
        ...
        {
            hp = GameManager.instance.mobSpawnManager.mobList[7].hp;
            time = 0;
            a = 0;
            moveSpeed = 0.3f;
